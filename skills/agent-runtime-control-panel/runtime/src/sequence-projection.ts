...
export function projectSequence(facts: SequenceFacts): SequenceProjection {
  ...
  for (const session of facts.sessions.filter((item) => include(item.workspaceId))) {
    ...
    add(`runtime:${session.id}:launched`, session.createdAt, 'runtime_launched', subject, `Runtime launched: ${session.provider}`, { runtimeId: session.id, generation: session.generation, provider: session.provider, hasContract: Boolean(session.taskId) }, { actor: actor(session.memberId, members), refs: [ref('goal', session.goalId), ...(session.taskId ? [ref('task', session.taskId)] : [])] });
    if (session.lastObservedAt) add(`runtime:${session.id}:observed`, session.lastObservedAt, session.state === 'terminal' ? 'runtime_terminal' : 'runtime_observed', subject, `Runtime ${session.state}`, session.state === 'terminal' ? { runtimeId: session.id, generation: session.generation } : { runtimeId: session.id, generation: session.generation, state: session.state }, { causal: [{ kind: 'caused_by', entryId: `runtime:${session.id}:launched` }] });
    ...
    if (previousGeneration) add(`runtime:${session.id}:generation`, session.createdAt, 'runtime_generation_changed', subject, `Runtime generation ${session.generation}`, { runtimeId: session.id, generation: session.generation }, { causal: [{ kind: 'replaces', entryId: `runtime:${previousGeneration.id}:launched` }] });
  }
  ...
  for (const delivery of facts.deliveries) {
    ...
    const base = { deliveryId: delivery.id, purpose: delivery.purpose ?? 'message', targetGeneration: delivery.generation };
