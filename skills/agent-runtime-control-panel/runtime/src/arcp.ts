...
export class PaseoAdapter implements RuntimeAdapter {
  ...
  launch(profile: Profile, goalTitle: string, workspace?: string, context?: RuntimeLaunchContext) {
    const handoff = context?.workspaceId && context.taskId && context.runtimeId
      ...
      : '';
    return this.cli.run(['run', '-d', '--json', '--provider', profile.provider, '--model', profile.model, ...(profile.mode ? ['--mode', profile.mode] : []), ...(profile.thinking ? ['--thinking', profile.thinking] : []), ...(workspace ? ['--cwd', workspace] : []), ...(context?.clientStatePath ? ['--env', `ARCP_CLIENT_STATE=${context.clientStatePath}`] : []), ...(context?.memberCredential ? ['--env', `ARCP_RUNTIME_MEMBER_CREDENTIAL=${context.memberCredential}`] : []), `Work on ARCP Goal: ${goalTitle}${handoff}`], { timeoutMs: 30_000 });
  }
