...
export class PaseoAdapter implements RuntimeAdapter {
  ...
  launch(profile: Profile, goalTitle: string, workspace?: string, context?: RuntimeLaunchContext) {
    const handoff = context?.workspaceId && context.taskId && context.runtimeId
      ...
      : '';
    return this.cli.run(['run', '-d', '--json', '--provider', profile.provider, '--model', profile.model, ...(profile.mode ? ['--mode', profile.mode] : []), ...(profile.thinking ? ['--thinking', profile.thinking] : []), ...(workspace ? ['--cwd', workspace] : []), `Work on ARCP Goal: ${goalTitle}${handoff}`], { timeoutMs: 30_000, env: { ...(context?.memberCredential ? { ARCP_RUNTIME_MEMBER_CREDENTIAL: context.memberCredential } : {}), ...(context?.clientStatePath ? { ARCP_CLIENT_STATE: context.clientStatePath } : {}) } });
  }
...
export class ArcpService {
  ...
  async resolveDecision(eventId: string, sourceMemberId: string, summary = 'Decision resolved'): Promise<ChannelEvent> {
    return this.store.mutate((state) => { const triggering = state.channelEvents.find((event) => event.id === eventId && event.kind === 'decision_required'); if (!triggering) throw new ArcpError('not_found', 'decision event not found'); triggering.decisionRequired = false; return this.appendChannelEvent(state, { workspaceId: triggering.workspaceId, goalId: triggering.goalId, taskId: triggering.taskId, resultId: triggering.resultId, sourceMemberId, targetMemberId: triggering.sourceMemberId, targetActorId: triggering.sourceActorId, kind: 'decision_resolved', urgency: 'normal', decisionRequired: false, summary, evidenceRefs: [], relatedEventId: eventId }); }).then(async (event) => { await this.pump(); return event; });
  }
