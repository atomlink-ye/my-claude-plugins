...
export async function handleArcp(req: http.IncomingMessage, res: http.ServerResponse, service: ArcpService): Promise<boolean> {
  ...
  try {
    ...
    if (method === 'POST' && path === '/v1/start') { if (!authenticatedActor) throw new ArcpError('unknown_sender', 'actor credential is required'); const input = await body(req); const started = await service.startManaged({ ...input, actorId: authenticatedActor.id } as any); send(res, 201, { goal: started.goal, task: started.task, member: publicMember(started.member), session: publicSession(started.session) }); return true; }
