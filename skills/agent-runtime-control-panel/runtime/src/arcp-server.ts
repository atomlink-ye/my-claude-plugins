...
export async function handleArcp(req: http.IncomingMessage, res: http.ServerResponse, service: ArcpService): Promise<boolean> {
  ...
  try {
    ...
    if (method === 'POST' && path === '/v1/participants/attach') { if (!authenticatedMember && !admin) throw new ArcpError('unauthorized', 'member credential is required'); const input = await body(req); const memberId = authenticatedMember ? authenticatedMember.id : String(input.memberId ?? ''); const workspaceId = authenticatedMember ? authenticatedMember.workspaceId : String(input.workspaceId ?? ''); send(res, 201, publicSession(await service.attachParticipant({ workspaceId, memberId, adapterId: String(input.adapterId ?? ''), externalId: String(input.externalId ?? ''), ...(input.workspace ? { workspace: String(input.workspace) } : {}), ...(input.taskId ? { taskId: String(input.taskId) } : {}) }))); return true; }
