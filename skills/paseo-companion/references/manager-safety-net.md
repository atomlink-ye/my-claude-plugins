...
| `POST /compact-wake` `{agentId, compact, wake?, delaySeconds?, waitForIdle?, trigger?}` | The coordinator sends `"/compact " + compact` itself — you no longer self-compact and then call this to arm a wake watcher. `wake` is optional: given, it is delivered once you're observed idle-and-stable again after the compact; omitted, nothing wakes you (compact-only). `waitForIdle` (default **true**) makes `delaySeconds` (default 10) an *idle-stability* window rather than a wall-clock delay: the coordinator holds the send until you have been `status=idle` with an unchanged `UpdatedAt` for that long, and any activity restarts the wait — so the `/compact` cannot land mid-turn. Verified live: registered with `delaySeconds:10` against an agent that stayed `running`, nothing was sent for 40s; the pre-fix pure-timer build truncated a turn in flight at 10s. Pass `waitForIdle:false` to opt back into the pure timer. Polling is on a 5s cadence, so a 10s window settles in ~10-15s. `DELETE /reminders/:id` on the returned `id` vetoes it at any point during the wait. Bounded at 1h, after which the record goes `dead` and logs `compact-wake-idle-timeout`. Without `trigger` the call itself is the trigger (fires once, after the wait). With `trigger:{metric:"context-percent",thresholdPercent?,thresholdTokens?,comparator?}` it becomes a standing subscription evaluated on the existing reconcile loop instead of firing immediately — and **crossing a threshold only ever notifies you; the coordinator never compacts on its own** (Owner 2026-08-19 rejected that outright, so there is no `mode` field at all: `mode:"auto"` and `mode:"notify"` both now 400 as unrecognized, and `waitForIdle` alongside a `trigger` 400s rather than being silently ignored). The response echoes `compact`/`wake`/`delaySeconds`/`trigger` plus `waitForIdle` (no-trigger) or `action:"notify-only"` (trigger) (with both `thresholdPercent` and `thresholdTokens`, normalized against the agent's currently observed max) so a caller never has to re-read the store to confirm what was actually registered. |
