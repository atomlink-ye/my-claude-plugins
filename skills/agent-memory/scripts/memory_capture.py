...
def _slug(text):
    return re.sub(r"[^a-z0-9]+", "-", text.lower()).strip("-")[:64] or "memory"
...
def _pick_root(binding, root=None):
    ...
    if root:
        ...
        matches = [x for x in roots if x == candidate]
        if len(matches) != 1:
            raise MemoryError(
                f"capture root is not configured for project {binding.project!r}: {candidate}"
            )
        return matches[0]
    if len(roots) != 1:
        raise MemoryError(
            f"project {binding.project!r} has multiple memory roots; pass --root explicitly: "
            + ", ".join(map(str, roots))
        )
    return roots[0]
...
def _related(note, items):
    ...
    for raw in items:
        ...
        out.append(
            f"- [{target.name}]({os.path.relpath(target,note.parent).replace(os.sep,'/')})"
        )
    return out


def find_capture_duplicates(binding: Binding, summary: str, threshold: float = 0.72):
                    ...
                    if end > 0:
                        for line in text[4:end].splitlines():
                            ...
                            if k.strip() == "title":
                                ...
                                brief = v
                candidate = (title or brief or p.stem).casefold()
...
def capture_memory(
    ...
    *,
    ...
    if kind not in KINDS:
        raise MemoryError(
            f"unsupported capture kind {kind!r}; choose from: {', '.join(KINDS)}"
        )
    ...
    n = 2
    while note.exists():
        note = folder / f"{stem}-{n}.md"
        n += 1
    mid = memory_id or new_memory_id()
    if not re.fullmatch(r"mem_[A-Za-z0-9_-]+", mid):
        raise MemoryError(f"invalid memory id: {mid!r}")
    canonical = _dedupe(
        _normalize_tags(
            [f"{binding.project}:{category}", f"self-improvement:{kind}", *tags]
        )
    )
    title = _scalar(summary)
    brief = title[:280]
    lines = [
        "---",
        f"id: {mid}",
    ...
    if details.strip():
        lines += ["", "## Why", "", details.strip()]
    if suggested_action.strip():
        lines += ["", "## How to apply", "", suggested_action.strip()]
    links = _related(note, related)
    if links:
        lines += ["", "## Related Files", "", *links]
    lines.append("")
