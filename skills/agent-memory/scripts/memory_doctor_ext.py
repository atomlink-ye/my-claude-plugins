...
def doctor(conn,settings,settings_path,db_path,target_path=None):
 ...
 for row in rows:
  path,mid,status,promoted,superseded=row
 ...
 try:
  for row in conn.execute("SELECT target_memory_id FROM memory_links"):
   if row[0] and row[0] not in ids:checks.append({"code":"memory_link_unresolved","severity":"error","message":f"memory:// target does not exist: {row[0]}"})
 except Exception:pass
 counts=Counter(x["severity"] for x in checks);r["status"]="error" if counts["error"] else "warn" if counts["warning"] else "ok";r["summary"]["errors"]=counts["error"];r["summary"]["warnings"]=counts["warning"];r["summary"]["info"]=counts["info"];r["summary"]["stable_ids"]=len(ids);checks.sort(key=lambda x:({"error":0,"warning":1,"info":2}[x["severity"]],x["code"]));return r
