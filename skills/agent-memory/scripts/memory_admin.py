...
from memory_config import Binding, MemoryError, SHARED_SCOPE, _expand_path, collect_memory_roots, flatten_bindings, resolve_binding
...
def _iter_binding_nodes(settings: dict[str, Any]) -> Iterable[tuple[dict[str, Any], Path]]:
    def visit(node: dict[str, Any], parent: Path | None) -> Iterable[tuple[dict[str, Any], Path]]:
        ...
        yield node, path
        for child in node.get("projects", []) if isinstance(node.get("projects", []), list) else []:
...
def _memory_entries(node: dict[str, Any] | None, binding_path: Path) -> list[tuple[Path, bool]]:
    if node is None:
        return []
    raw = node.get("memory", [])
    if raw is None:
...
def preferred_capture_root(settings: dict[str, Any], binding: Binding) -> Path | None:
    marked = [path for path, capture in _memory_entries(_binding_node(settings, binding), binding.path) if capture]
    if len(marked) > 1:
        raise MemoryError(
...
def project_inventory(conn: sqlite3.Connection, settings: dict[str, Any]) -> list[dict[str, Any]]:
    counts = {row["scope"]: int(row["count"]) for row in conn.execute("SELECT scope,count(DISTINCT document_id) AS count FROM document_scopes GROUP BY scope")}
    result = []
...
def tag_inventory(conn: sqlite3.Connection, project: str | None = None, include_shared: bool = True) -> list[dict[str, Any]]:
...
def _finding(code: str, severity: str, message: str, *, project: str | None = None, paths: Iterable[str | Path] = (), suggested_action: str | None = None) -> dict[str, Any]:
...
def doctor(conn: sqlite3.Connection, settings: dict[str, Any], settings_path: Path, db_path: Path, target_path: Path | None = None) -> dict[str, Any]:
    ...
    for binding in bindings:
        ...
        if not binding.memory_roots:
            findings.append(_finding("project_has_no_memory_root", "warning", "project has no configured memory root", project=binding.project, paths=[binding.path]))
        entries = _memory_entries(_binding_node(settings, binding), binding.path)
        marked = [path for path, capture in entries if capture]
        if len(marked) > 1:
            findings.append(_finding("capture_root_multiple", "error", "project has multiple memory roots marked capture=true", project=binding.project, paths=marked, suggested_action="leave capture=true on exactly one memory root"))
        elif len(binding.memory_roots) > 1 and not marked:
            findings.append(_finding("capture_root_ambiguous", "warning", "project has multiple memory roots and no default capture root", project=binding.project, paths=[location.path for location in binding.memory_roots], suggested_action="mark one memory root with capture=true"))
        elif marked and not _directory_writable(marked[0]):
            findings.append(_finding("capture_root_not_writable", "error", "configured capture root is not writable", project=binding.project, paths=marked))

    for path, values in by_path.items():
        if len(values) > 1:
            findings.append(_finding("binding_path_duplicate", "error", "multiple project bindings resolve to the same filesystem path", paths=[path]))
    for project, values in by_project.items():
        unique_paths = sorted({binding.path for binding in values}, key=str)
        if len(unique_paths) > 1:
            findings.append(_finding("project_name_reused", "warning", "the same project name is bound to multiple filesystem paths", project=project, paths=unique_paths))

    physical_scopes: dict[Path, set[str]] = defaultdict(set)
    for root in roots:
        physical_scopes[root.path].add(root.scope)
        if not root.path.exists():
            findings.append(_finding("memory_root_missing", "warning", "configured memory root does not exist", project=None if root.shared else root.scope, paths=[root.path], suggested_action="create the directory or remove the stale settings entry"))
    ...
    for path, scopes in physical_scopes.items():
        ...
        if len(project_scopes) > 1:
            findings.append(_finding("memory_root_multi_scope", "warning", "one physical memory root is visible in multiple project scopes", paths=[path], suggested_action="verify this overlap is intentional; use shared roots for intentionally global knowledge"))
    ...
    for row in indexed_rows:
        ...
        if not path.exists():
            stale += 1
            findings.append(_finding("indexed_file_missing", "warning", "indexed Markdown source no longer exists", paths=[path], suggested_action="run agent-memory sync"))
        else:
            stat = path.stat()
            changed += int(stat.st_mtime_ns != int(row["mtime_ns"]) or stat.st_size != int(row["size"]))
    if changed:
        findings.append(_finding("index_source_changed", "warning", f"{changed} indexed Markdown file(s) changed since the last sync", suggested_action="run agent-memory sync"))
    ...
    if unindexed:
        findings.append(_finding("markdown_unindexed", "warning", f"{len(unindexed)} Markdown file(s) under configured roots are not indexed", paths=unindexed[:20], suggested_action="run agent-memory sync"))
    ...
    for variants in case_groups.values():
        if len(variants) > 1:
            findings.append(_finding("tag_case_collision", "warning", "tags differ only by case", paths=sorted(variants)))
    for variants in separator_groups.values():
        if len(variants) > 1:
            findings.append(_finding("tag_separator_collision", "warning", "tags differ only by hyphen/underscore separators", paths=sorted(variants)))
    ...
    return {
        ...
        "summary": {
            ...
            "bindings": len(bindings), "documents": len(indexed_rows), "dangling_links": dangling,
            "unindexed_markdown": len(unindexed), "stale_documents": stale,
        },
        "resolved": resolved,
        "checks": findings,
    }
