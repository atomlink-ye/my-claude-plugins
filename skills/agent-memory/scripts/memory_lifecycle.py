...
def _set_fields(path:Path,fields:dict[str,str|None],create_frontmatter=False):
 ...
 if not text.startswith("---\n"):
  if not create_frontmatter:raise MemoryError(f"lifecycle update requires frontmatter: {path}")
  prefix=[f"{k}: {v}" for k,v in fields.items() if v is not None];path.write_text("---\n"+"\n".join(prefix)+"\n---\n\n"+text,encoding="utf-8");return
 end=text.find("\n---\n",4)
...
def migrate_ids(conn):
 changed=[]
 for row in conn.execute("SELECT d.path FROM documents d LEFT JOIN memory_meta m ON m.document_id=d.id WHERE m.memory_id IS NULL OR m.memory_id='' ").fetchall():
  path=Path(row[0]);mid=new_memory_id();_set_fields(path,{"id":mid},create_frontmatter=True);changed.append({"path":str(path),"memory_id":mid})
