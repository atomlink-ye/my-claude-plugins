...
def main():
    p = argparse.ArgumentParser(
        description="Assign stable IDs to legacy Agent Memory Markdown explicitly"
    )
    ...
    c = connect_db(database_path(settings, sp))
    sync_index(c, collect_memory_roots(settings, sp))
    r = migrate_ids(c)
    ...
    print(
        ...
        if a.json
        else f"migrated {r['migrated']} document(s)"
    )
