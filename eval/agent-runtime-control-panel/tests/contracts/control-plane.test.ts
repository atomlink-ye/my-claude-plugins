...
describe('ARCP MVE control core', () => {
  ...
  it('keeps same-checkout Tabs visible, gates a real writer session, and archives only an explicit disposable lane', async () => {
    ...
    await service.submitResult({ workspaceId: workspace.workspace.id, taskId: writer.task.id, memberId: writer.member.id, status: 'candidate', summary: 'handoff complete', expectedFence: 1, runtimeSessionId: writer.session.id, runtimeGeneration: writer.session.generation });
  ...
  it('repairs a missing externalId from one durable placement identity before reconciling', async () => {
    const root = await mkdtemp(path.join(os.tmpdir(), 'arcp-reconcile-placement-')); const service = await control(root); const { actor } = await service.registerActor({ clientIdentity: 'placement-reconcile-owner' }); const workspace = await service.createWorkspace({ ownerActorId: actor.id, purpose: 'placement reconcile' }); const started = await service.startManaged({ actorId: actor.id, workspaceId: workspace.workspace.id, title: 'placement recovery', profileId: 'codex-worker' }) as any;
    await service.store.mutate((state: any) => { const session = state.sessions.find((item: any) => item.id === started.session.id); delete session.externalId; session.placement.requested.agentId = 'paseo-session-1'; for (const binding of state.runtimeBindings.filter((item: any) => item.runtimeSessionId === session.id)) delete binding.nativeId; });
    expect(await service.reconcile(started.session.id)).toMatchObject({ externalId: 'paseo-session-1', state: 'idle' }); service.close();
  });
  it('fails closed instead of overwriting conflicting durable identities', async () => {
    const root = await mkdtemp(path.join(os.tmpdir(), 'arcp-reconcile-identity-conflict-')); const service = await control(root); const { actor } = await service.registerActor({ clientIdentity: 'identity-conflict-owner' }); const workspace = await service.createWorkspace({ ownerActorId: actor.id, purpose: 'identity conflict' }); const started = await service.startManaged({ actorId: actor.id, workspaceId: workspace.workspace.id, title: 'identity conflict', profileId: 'codex-worker' }) as any;
    await service.store.mutate((state: any) => state.runtimeBindings.filter((item: any) => item.runtimeSessionId === started.session.id).forEach((item: any) => { item.nativeId = 'other-agent'; }));
    expect(await service.reconcile(started.session.id)).toMatchObject({ state: 'transport_indeterminate' }); expect(service.state().runtimeBindings.filter((item) => item.runtimeSessionId === started.session.id).every((item) => item.nativeId === 'other-agent' && item.state === 'transport_indeterminate')).toBe(true); service.close();
  });
  it('reconciles provider identity and settings from a snapshot, then converges bindings', async () => {
    const root = await mkdtemp(path.join(os.tmpdir(), 'arcp-reconcile-settings-')); const service = await control(root); const { actor } = await service.registerActor({ clientIdentity: 'settings-reconcile-owner' }); const workspace = await service.createWorkspace({ ownerActorId: actor.id, purpose: 'settings reconcile' }); const started = await service.startManaged({ actorId: actor.id, workspaceId: workspace.workspace.id, title: 'settings drift', profileId: 'codex-worker' }) as any;
    await service.store.mutate((state: any) => state.runtimeBindings.push({ id: 'second-binding', executionSurfaceId: 'surface-2', runtimeSessionId: started.session.id, generation: started.session.generation, adapterId: 'paseo', nativeId: 'paseo-session-1', state: 'running', createdAt: new Date().toISOString() }));
    (service.cli as any).inspectValue = { model: 'other-model' }; expect(await service.reconcile(started.session.id)).toMatchObject({ state: 'attention', observed: { model: 'other-model' } }); expect(service.state().runtimeBindings.filter((item) => item.runtimeSessionId === started.session.id).every((item) => item.state === 'attention')).toBe(true);
    (service.cli as any).inspectValue = { id: 'other-agent' }; expect(await service.reconcile(started.session.id)).toMatchObject({ state: 'placement_mismatch' }); service.close();
  });
  it('does not append uncertainty facts when generation changes before their durable mutation', async () => {
    const root = await mkdtemp(path.join(os.tmpdir(), 'arcp-reconcile-uncertainty-race-')); const service = await control(root, false, ['codex'], {}, undefined, undefined, [{ id: 'paseo-session-1', status: 'unknown' }]); const { actor } = await service.registerActor({ clientIdentity: 'uncertainty-race-owner' }); const workspace = await service.createWorkspace({ ownerActorId: actor.id, purpose: 'uncertainty race' }); const started = await service.startManaged({ actorId: actor.id, workspaceId: workspace.workspace.id, title: 'uncertainty race', profileId: 'codex-worker' }) as any;
    const emit = (service as any).emitTransportUncertainty.bind(service); (service as any).emitTransportUncertainty = async (session: any) => { await service.store.mutate((state: any) => { const current = state.sessions.find((item: any) => item.id === session.id); current.generation += 1; current.state = 'transport_indeterminate'; }); await emit(session); };
    await service.reconcile(started.session.id); expect(service.channelEvents(workspace.workspace.id).filter((event) => ['runtime_health', 'transport_uncertainty'].includes(event.kind))).toHaveLength(0); service.close();
  });
  it('does not terminalize a live external runtime from an unrelated empty registry', async () => {
    const root = await mkdtemp(path.join(os.tmpdir(), 'arcp-reconcile-external-live-')); const service = await control(root); const { actor, binding } = await service.registerActor({ clientIdentity: 'external-live-owner' }); const workspace = await service.createWorkspace({ ownerActorId: actor.id, purpose: 'external live' });
    service.registerAdapter({ adapterId: 'hermes-acp', discover: async () => ({ value: [], stdout: '', stderr: '' }), models: async () => ({ value: [], stdout: '', stderr: '' }), modes: async () => [], launch: async () => ({ value: {}, stdout: '', stderr: '' }), observe: async () => ({ value: {}, stdout: '', stderr: '' }), snapshot: async (externalId: string) => ({ agent: { id: externalId, status: 'idle', provider: 'hermes', model: 'hermes-agent' }, timeline: [], source: 'cli' }), registry: async () => ({ value: [], stdout: '', stderr: '' }), providerSubagents: async () => ({ source: 'none', items: [] }), startTurn: async () => undefined, interrupt: async () => undefined, stop: async () => undefined, reconcileExternal: async () => true } as any);
    await service.store.mutate((state: any) => state.sessions.push({ id: 'external-live', actorId: actor.id, goalId: 'external-goal', bindingId: binding.id, generation: 1, runtimeKind: 'external', adapterId: 'hermes-acp', workspaceId: workspace.workspace.id, memberId: workspace.member.id, profileId: 'hermes-acp', provider: 'hermes', model: 'hermes-agent', state: 'running', externalId: 'acp-live', createdAt: new Date().toISOString() })); expect(await service.reconcile('external-live')).toMatchObject({ state: 'idle', externalId: 'acp-live' }); service.close();
  });
  ...
  it('persists an ACP-created Result and delivers its decision request to the manager channel', async () => {
    ...
    const acp = await adapter.launch({ id: 'hermes-acp', provider: 'hermes', model: 'hermes-agent', role: 'worker' }, 'ACP result', '.', { runtimeId: 'worker-runtime', runtimeGeneration: 1, taskId: task.id, memberId: worker.member.id }); const externalId = String((acp.value as any).id);
  ...
  it('records the complete ChannelEvent kind inventory with task/result linkage', async () => {
    const root = await mkdtemp(path.join(os.tmpdir(), 'arcp-channel-inventory-')); const service = await control(root); await service.store.mutate((state: any) => { state.tasks.push({ id: 'task-inventory', workspaceId: 'workspace-inventory', title: 'inventory', lifecycle: 'completed', fence: 1, createdAt: new Date().toISOString(), updatedAt: new Date().toISOString() }); state.results.push({ id: 'result-4', workspaceId: 'workspace-inventory', taskId: 'task-inventory', memberId: 'member-inventory', fence: 1, status: 'candidate', summary: 'inventory candidate', evidenceRefs: [], createdAt: new Date().toISOString() }); }); const kinds: any[] = ['decision_required', 'decision_resolved', 'task_claimed', 'task_candidate', 'task_completed', 'task_failed', 'task_unknown', 'phase_progress', 'phase_completed', 'blocker', 'finding', 'permission', 'attention', 'runtime_health', 'transport_uncertainty', 'material_progress', 'workspace_analysis_required'];
  ...
  it('uses a role as routing intent and explains the fixed provider selection without fallback', async () => {
    ...
    expect(worker).toMatchObject({ action: 'launch', profileId: 'codex-worker', selection: { roleIntent: 'worker', selection: 'role-intent', selectedProfile: { id: 'codex-worker', provider: 'codex', role: 'worker' } } });
    expect(worker.selection.alternatives).toEqual(expect.arrayContaining([expect.objectContaining({ id: 'pi-grok-worker', provider: 'pi', reason: expect.stringContaining('select it explicitly') })]));
    const manager = await service.preflight({ role: 'manager' });
    expect(manager).toMatchObject({ action: 'launch', profileId: 'claude-manager', selection: { roleIntent: 'manager', selection: 'role-intent', selectedProfile: { provider: 'claude', role: 'manager' } } });
    const explicit = await service.preflight({ role: 'manager', provider: 'codex', model: 'gpt-5.6-terra', mode: 'auto', thinking: 'medium' });
    expect(explicit).toMatchObject({ action: 'launch', profileId: 'explicit', selection: { roleIntent: 'manager', selection: 'explicit-settings', selectedProfile: { provider: 'codex', role: 'explicit' }, alternatives: [] } });
    const unavailable = await control(await mkdtemp(path.join(os.tmpdir(), 'arcp-role-no-fallback-')), false, ['pi']);
    await expect(unavailable.preflight({ role: 'worker' })).resolves.toMatchObject({ action: 'hold', profileId: 'codex-worker', selection: { selection: 'role-intent' } });
...
describe('ARCP Slice A channel correctness', () => {
  ...
  it('does not replay an already-delivered event after service restart', async () => {
    ...
    const second = await control(root); expect((second.cli as any).sends).toBe(0); expect(second.state().deliveries.find((item) => item.id === delivery.id)?.state).toBe('delivered'); second.close();
  });
  ...
  it('awaits restart pump and distinguishes delivered history from waiting safe-point work', async () => {
    ...
    const second = await control(root); expect((second.cli as any).sends).toBe(1); expect(second.state().deliveries.find((item) => item.id === 'delivered-history')?.state).toBe('delivered'); expect(second.state().deliveries.find((item) => item.id === 'waiting-work')?.state).toBe('delivered'); second.close();
  });
});
...
describe('ARCP HTTP surface', () => {
  ...
  it('recursively redacts nested provider handles and private paths from session, list, and panorama projections', async () => {
    ...
    // handle and pathLeak sit under key names redactProviderValue already knows,
    // so they alone cannot tell an allowlist projection apart from the old
    // denylist. futureLeak is the discriminating case: it hides under key names
    // no denylist could anticipate, at the top level and inside observed. Only
    // an explicit projection drops it, which is the whole point of the change.
...
describe('ARCP CLI and TUI presentation', () => {
  it('keeps two-word command parsing, flag values, auth, retirement and HTTP exits deterministic', async () => {
    ...
    await execFileAsync(process.execPath, [script, 'canary', '--workspace', 'workspace-1'], { cwd: path.join(process.cwd(), '..'), env: { ...process.env, ARCP_URL: `http://127.0.0.1:${port}`, ARCP_API_KEY: 'cli-key', ARCP_CLIENT_STATE: path.join(await mkdtemp(path.join(os.tmpdir(), 'arcp-cli-')), 'client.json') } });
    expect(seen.slice(-2)).toMatchObject([{ url: '/v1/discovery', key: 'cli-key' }, { url: '/v1/workspaces/workspace-1/context', key: 'cli-key' }]);
    await execFileAsync(process.execPath, [script, 'escalate', 'event-positional', '--reason', 'ACK SLA expired'], { cwd: path.join(process.cwd(), '..'), env: { ...process.env, ARCP_URL: `http://127.0.0.1:${port}`, ARCP_API_KEY: 'cli-key', ARCP_CLIENT_STATE: path.join(await mkdtemp(path.join(os.tmpdir(), 'arcp-cli-')), 'client.json') } });
