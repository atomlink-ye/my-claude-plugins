...
describe('provider budget MVE', () => {
  ...
  it('keeps Claude and Codex available when the independent Grok collector fails', async () => {
    const dir = await mkdtemp(path.join(os.tmpdir(), 'arcp-codexbar-')); const script = path.join(dir, 'collector');
    await writeFile(script, `#!${process.execPath}\nif(process.argv.includes('grok'))process.exit(2);const provider=process.argv.includes('claude')?'claude':'codex';process.stdout.write(JSON.stringify([{provider,usage:{updatedAt:new Date().toISOString(),primary:{usedPercent:15},secondary:{usedPercent:25}}}]));\n`); await chmod(script, 0o755);
    const prior = process.env.ARCP_CODEXBAR_BIN; process.env.ARCP_CODEXBAR_BIN = script;
    try { const snapshot = await collectCodexbar(); expect(snapshot.providers).toEqual(expect.arrayContaining([expect.objectContaining({ providerId: 'claude', status: 'available' }), expect.objectContaining({ providerId: 'codex', status: 'available' }), expect.objectContaining({ providerId: 'pi', status: 'error', error: 'provider collector failed' })])); expect(evaluateAdmission({ envelope: snapshot, bindings: [{ id: 'grok', providerId: 'pi', sourceId: 'local-codexbar', modelPatterns: ['grok-cli/grok-4.6'], windowIds: ['primary'], admissionPolicyId: 'default' }], policies, providerId: 'pi', model: 'grok-cli/grok-4.6' }).action).toBe('hold_unknown'); }
    finally { if (prior === undefined) delete process.env.ARCP_CODEXBAR_BIN; else process.env.ARCP_CODEXBAR_BIN = prior; }
  });
  it('translates a redacted real-shape CodexBar Grok observation to Pi admission', async () => {
    const dir = await mkdtemp(path.join(os.tmpdir(), 'arcp-codexbar-grok-')); const script = path.join(dir, 'collector'); const updatedAt = new Date().toISOString();
    await writeFile(script, `#!${process.execPath}\nconst provider=process.argv.includes('claude')?'claude':process.argv.includes('codex')?'codex':'grok';const usage=provider==='grok'?{updatedAt:${JSON.stringify(updatedAt)},primary:{usedPercent:19,resetsAt:'2026-09-05T17:21:31Z'},plan:'SuperGrok',email:'owner@example.com',organization:'private-org',login:'private-login'}:{updatedAt:${JSON.stringify(updatedAt)},primary:{usedPercent:15},secondary:{usedPercent:25}};process.stdout.write(JSON.stringify([{provider,source:provider==='grok'?'grok-cli-proxy':'local',version:'1.0.13',usage}]))\n`); await chmod(script, 0o755);
    const prior = process.env.ARCP_CODEXBAR_BIN; process.env.ARCP_CODEXBAR_BIN = script;
    try { const snapshot = await collectCodexbar(); const grok = snapshot.providers.find((provider) => provider.providerId === 'pi')!; expect(grok).toMatchObject({ providerId: 'pi', status: 'available', sourceLabel: 'codexbar:grok', windows: [expect.objectContaining({ id: 'primary', usedPct: 19, remainingPct: 81, resetsAt: '2026-09-05T17:21:31Z' })] }); expect(JSON.stringify(snapshot)).not.toMatch(/owner@example\.com|private-org|private-login/); expect(evaluateAdmission({ envelope: snapshot, bindings: [{ id: 'grok', providerId: 'pi', sourceId: 'local-codexbar', modelPatterns: ['grok-cli/grok-4.6'], windowIds: ['primary'], admissionPolicyId: 'default' }], policies, providerId: 'pi', model: 'grok-cli/grok-4.6' }).action).toBe('launch'); }
    finally { if (prior === undefined) delete process.env.ARCP_CODEXBAR_BIN; else process.env.ARCP_CODEXBAR_BIN = prior; }
  });
