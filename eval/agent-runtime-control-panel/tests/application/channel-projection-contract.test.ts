...
describe('Channel projection application contract', () => {
  it('uses one canonical projection across channel list, inbox, panorama/TUI and the delivery envelope', async () => {
    ...
    const result = await service.submitResult({ workspaceId, taskId: task.id, memberId: worker.member.id, status: 'candidate', summary: 'Task completed by durable Result', expectedFence: 1 });
    ...
    const event = await service.publishChannelEvent({ workspaceId, taskId: task.id, resultId: result.id, sourceMemberId: worker.member.id, targetMemberId: manager.member.id, kind: 'task_completed', urgency: 'normal', consumptionPolicy: 'ack_required', decisionRequired: false, summary: `Task ${task.id} completed by durable Result`, evidenceRefs: [] });
