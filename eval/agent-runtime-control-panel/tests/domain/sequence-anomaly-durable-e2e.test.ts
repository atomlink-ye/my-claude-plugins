...
describe('Sequence anomaly projection from durable ArcpService data', () => {
  ...
  it('detects contract_after_start and candidate_before_contract from a late durable Contract Delivery', async () => {
    ...
    try {
      ...
      await service.submitResult({ workspaceId: workspace.id, taskId: started.task.id, memberId: started.member.id, status: 'candidate', summary: 'candidate after late contract', evidenceRefs: ['commit:durable'], expectedFence: 1, runtimeSessionId: started.session.id, runtimeGeneration: started.session.generation });
